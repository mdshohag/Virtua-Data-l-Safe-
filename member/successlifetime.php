<?php 
	...
	$date = new DateTime();
	$addoneyeardate = $date->modify('+99 years')->format('Y-m-d');
	//$addoneyeardate = date("Y-m-d", strtotime("+99 year"));
	...
	$r7 = $show->fetch_assoc();
	$s_paid = $r7['lifetime_pay'];
	$vs_code = $r7['vs_code'];
	
	//$result = $db->query("SELECT * FROM tbl_vs_client WHERE subscription_rcbl = '$testpayment' AND client_id = '$client_id'");
	$result = $db->query("UPDATE tbl_vs_client SET valid_from='$todate', valid_to='$addoneyeardate', status='Lifetime' WHERE lifetime_pay = '$testpayment' AND client_id = '$client_id'");
...
// ai khane check kora hobe
	...
	$resultAuditTable = $db->query("insert into tbl_audit_trail (table_name,field_name,old_value,new_value,change_by,change_date) values ('tbl_vs_client','lifetime_pay','$s_paid','$testpayment','$client_id','$todate')");
	...
	if($result && $affected_rows > 0 && $resultMasterTable && $resultAuditTable && $resultvscmasTable)
	{
		...
		//echo '1';
		echo("<script>location.href = 'http://localhost/benefits_hts/member/vscodelife.php';</script>");
		die;
